...
def __init():
    ...
    # file.
    __background = __canvas.create_rectangle(0, 0, getWidth() + 1, getHeight() + 1, fill=__bgcolor, outline=__bgcolor,
                                             tag="__background")
...
def line(*pts):
    try:
        ...
        __canvas.create_line(new_pts, fill=__outline, width=__width, capstyle=__capstyle, joinstyle=__joinstyle,
                             arrow=__arrow, arrowshape=__arrowshape)
        __update()
...
def curve(*pts):
    try:
        ...
        __canvas.create_line(new_pts, fill=__outline, width=__width, capstyle=__capstyle, smooth=True, splinesteps=25,
                             joinstyle=__joinstyle, arrow=__arrow, arrowshape=__arrowshape)
        __update()
...
def rect(x, y, w, h):
    ...
    try:
        if abs(w) >= 2 and abs(h) >= 2:
            __canvas.create_rectangle(x + 1, y + 1, x + 1 + w - 1, y + 1 + h - 1, fill=__fill, outline=__outline,
                                      width=__width)
            __update()
        elif abs(w) == 1:
            line(x, y, x, y + h - 1)
...
def setFont(f=None, s=10, modifiers=""):
    ...
    if f == None:
        ...
        try:
            ...
            __font = font.Font(family=f, size=s, name=str(__font_count), weight=w, slant=sl, underline=und,
                               overstrike=ovs)
            __font_count += 1
            return True
        except Exception as e:
...
def arc(x, y, w, h, s, e):
    try:
        __canvas.create_arc(x + 1, y + 1, x + 1 + w, y + 1 + h, start=s, extent=e, fill=__fill, outline=__outline,
                            style=tk.ARC, width=__width)
        __update()
...
def pieSlice(x, y, w, h, s, e):
    try:
        __canvas.create_arc(x + 1, y + 1, x + 1 + w, y + 1 + h, start=s, extent=e, fill=__fill, outline=__outline,
                            style=tk.PIESLICE, width=__width)
        __update()
...
def clear():
    try:
        __canvas.delete("all")
        __background = __canvas.create_rectangle(0, 0, getWidth(), getHeight(), fill=__bgcolor, outline=__bgcolor,
                                                 tag="__background")
    except AttributeError:
        pass;
